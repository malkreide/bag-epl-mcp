...
Freigabeliste nannte `Mcp-Session-Id`, `Content-Type` und `Authorization` — den
Header der Session-Mechanik, die dieselbe Revision abgeschafft hat, aber keinen
der drei neuen.

Ein Browser darf einen nicht safelisteten Header gar nicht erst senden, wenn der
Server ihn nicht nennt: der Preflight endete mit 400, bevor ein MCP-Byte floss.
stdio- und Python-Clients kennen keinen Preflight und liefen weiter — deshalb
war die Suite gruen, waehrend jeder Browser-Client ausgesperrt war.
...
from bag_epl_mcp.server import CORS_ROUTING_HEADERS, _build_http_app, settings
...
def preflight(client: TestClient, announced: str):
    ...
    Der Header muss auf der Anfrage stehen: Starlette beantwortet einen
    Preflight, der einen nicht erlaubten Header nennt, mit 400 — das ist die
    Ablehnung, um die es geht, und sie ist an der Antwort allein nicht ablesbar.
    """
...
async def test_kein_tool_schema_verlangt_einen_mcp_param_header() -> None:
    ...
    eine `x-mcp-header`-Annotation. CORS kennt keinen Praefix-Wildcard, das
    erste Tool mit so einer Annotation muss den Header einzeln freigeben."""
